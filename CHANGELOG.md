...
- P93: уникальность ИНН перенесена из best-effort availability-check в БД — один реальный ИНН не может принадлежать двум пользовательским аккаунтам;
- optional ИНН при записи канонизируется: пробелы убираются, отсутствие значения хранится как `NULL`, поэтому множество пользователей без ИНН не конфликтуют;
- миграция fail-closed проверяет существующие дубли по `btrim(inn)`, не объединяет аккаунты автоматически и только после проверки создаёт unique index;
- исторические пустые/пробельные optional ИНН безопасно канонизируются в `NULL`.
