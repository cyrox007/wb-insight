...
- P99: регистрация преобразует в `409 REGISTRATION_CONFLICT` только известные уникальные ограничения email/телефона/ИНН; посторонние `IntegrityError` больше не маскируются под duplicate identity и проходят в общий безопасный 500-контур;
