...
- P84: Control Panel получил отдельное необратимое удаление пользователя после предварительной деактивации;
- permanent delete защищён отдельным разрешением `users:delete`, доступным только `admin` и `super_admin`; self-delete запрещён, а `super_admin` сохраняет существующую sensitive-target защиту;
- перед удалением требуется вручную подтвердить email пользователя; UI показывает отдельную опасную модалку и не смешивает permanent delete с обычной деактивацией;
- lifecycle evidence сохраняет идентификатор удалённого аккаунта без email, а связанные пользовательские данные удаляются штатными DB `ON DELETE CASCADE/SET NULL`;
- `user_service.py` приведён к request-транзакции: ошибки update/delete/role flush больше не маскируются как `None/False`.
