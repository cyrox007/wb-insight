...
- P92: `/auth/registration` теперь сам строго валидирует JSON-структуру и `registrationData`, не полагаясь на frontend/preflight;
- сервер до savepoint нормализует email/телефон и проверяет тип пользователя, ФИО/название, пароль, ИНН, КПП, юридический адрес, timezone и boolean-флаг рассылки;
- malformed JSON, неправильные JSON-типы, слишком длинные строки и пароль вне bcrypt-лимита получают предсказуемый `400`, не доходя до SQLAlchemy/хеширования;
- regression-тесты разделяют input-validation и атомарность регистрации, сохраняя проверку savepoint/rollback.
