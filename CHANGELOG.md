...
- подключение нового кабинета Wildberries больше не ждёт 10-минутного discovery: состояния и первичные задачи синхронизации создаются сразу в транзакции подключения; ошибки сохранения кабинета больше не скрываются, а устаревший `backend/services/sync.py` удалён;
