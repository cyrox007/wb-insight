...
- профиль показывает ожидающий подтверждения адрес и позволяет повторно отправить подтверждение; телефон честно помечен как изменяемый только администратором до появления отдельного подтверждаемого phone-flow;
- frontend contract test блокирует возврат ложной подсказки о self-service подтверждении телефона.
