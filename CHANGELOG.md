...
- P94: ИНН пользователя получает DB-уникальность на уровне непустого `trim(inn)`, чтобы параллельные регистрации не могли обойти availability-check;
...
- partial functional index не ограничивает пользователей без ИНН и не требует автоматически переписывать исторические значения;
- затронутая модель пользователей приведена к русскому системному контракту для человекочитаемой ошибки роли и комментария session version.
