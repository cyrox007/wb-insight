...
- P108: карточочная сетка тарифов заменена компактным responsive-списком с одной кнопкой действий на строку; системный demo визуально отделён от обычных тарифов;
- удаление тарифа оформлено как lifecycle «деактивировать → подтвердить код → удалить навсегда»; активный и системный тарифы backend удалить не позволяет;
- перед удалением backend проверяет исторические ссылки из подписок и платежей; используемый тариф остаётся неактивным и возвращает понятный 409 вместо ошибки внешнего ключа;
- frontend показывает причину блокировки удаления и количество связанных подписок/платежей, когда тариф уже использовался;

- P107: экран ролей упрощён: первым показываются только фактически назначенные роли пользователей, а техническая permission-матрица перенесена в раскрываемый справочный блок;
- удалён предвыбранный select будущей роли из таблицы; изменение выполняется отдельной модалкой «Изменить роли», где явно разделены текущие роли и добавление новой;
- назначение super_admin требует точного подтверждения email целевого пользователя и дополнительно проверяется backend-ом;
- карточка пользователя использует тот же единый редактор ролей; устаревший assign_role.vue удалён;

- P106: список пользователей Control Panel переработан для компактного desktop/mobile отображения: действия сведены к одной кнопке, тип аккаунта и роли объединены, дата регистрации больше не ломает ширину таблицы;
- фильтры применяются автоматически с debounce поиска; добавлен отдельный фильтр типа аккаунта, а поиск расширен до UUID, ИНН, отдела и должности;
- backend получил allowlist-сортировку asc/desc по имени, email, телефону, типу, статусу и дате регистрации; frontend показывает направление сортировки в заголовках;
- действие «Удалить…» теперь видно до деактивации и ведёт через понятный двухшаговый мастер: деактивация → точное подтверждение email → необратимое удаление;
- поздние ответы автофильтрации отбрасываются по поколению запроса, чтобы старый результат не перезаписывал новый фильтр;

- P105: список пользователей Control Panel получил явные действия деактивации, повторной активации и необратимого удаления с подтверждением email; destructive-действия больше не спрятаны только в карточке пользователя;
- суперадминистратор может создать служебный аккаунт с временным паролем и ролью admin/manager/support/analyst; создание super_admin остаётся отдельным защищённым role-management действием;
- клиентские аккаунты намеренно не создаются административной формой: обычная регистрация остаётся единственным контуром фиксации пользовательских юридических согласий;
- backend-контракт создания staff-аккаунта проверяет email/телефон/пароль/роль, уникальность идентификаторов и журналирует административное создание;

- P104: восстановлена совместимость с WB-токенами, сохранёнными до перехода 30 апреля 2026 года на отдельный производный ключ пользователя; расшифровка сначала использует текущую схему, затем безопасный legacy fallback;
- при успешной ручной проверке legacy-токен автоматически перешифровывается текущей схемой `master key + user_id`; повреждённый или уже нерасшифровываемый ciphertext возвращает понятную ошибку замены токена вместо `500`;
- тесты закрепляют текущий round-trip, чтение исторического Fernet ciphertext и fail-safe обработку нечитаемой записи;

- P103: устранён тупик seller analytics для staff/super_admin: состояние недоступного кабинета теперь ведёт прямо во вкладку «Кабинеты WB», которая доступна рабочему аккаунту без открытия клиентских расчётных вкладок;
- в управлении подключениями добавлена ручная повторная проверка сохранённого токена через Wildberries API; подтверждённый отозванный или истёкший токен получает фактический статус, временная недоступность WB не портит сохранённое состояние;
- исправлены русские формулировки количества проблемных подключений и добавлены понятные действия для добавления, проверки и удаления токена;

- P102: на главном дашборде появился мастер первого запуска, который ведёт нового продавца через подключение Wildberries, первичную синхронизацию и необязательное добавление себестоимости;
- отсутствие подключённого кабинета больше не выглядит как общая ошибка аналитики: пользователь получает отдельное состояние с прямым переходом к подключению;
- прогресс первичной синхронизации автоматически обновляется, а аналитика открывается после появления первых пригодных данных, не требуя ждать завершения всех источников;
- форма подключения WB получила короткую пошаговую инструкцию, перечень минимальных read-only категорий и человекочитаемую диагностику ошибок токена;
- профиль поддерживает глубокие ссылки на вкладки подключений, себестоимости и расходов; frontend-контракт закрепляет новый onboarding-flow.
...
- P89: регистрация пользователя переведена на единый атомарный savepoint — пользователь, базовая роль, юридические согласия, opt-out рассылки и demo/verification-mail больше не могут частично сохраниться;
- duplicate/unique conflict откатывает только savepoint и возвращает прежний `409`, а внутренние ошибки продолжают проходить через общий request rollback;
- P99: регистрация преобразует в `409 REGISTRATION_CONFLICT` только известные уникальные ограничения email/телефона/ИНН; посторонние `IntegrityError` больше не маскируются под конфликт пользовательских идентификаторов и проходят в общий безопасный 500-контур;
- классификатор учитывает текущие имена уникальных индексов и драйверные `constraint_name` / `diag.constraint_name`; regression-тесты закрепляют rollback только nested savepoint без ручного rollback request-сессии;
- P100: удалена публичная legacy-заглушка `GET /users/`, которая без авторизации возвращала пустой объект и не имела продуктового назначения;
- управление пользователями остаётся только в защищённом `/control-panel/users`, а regression-тест закрепляет отсутствие `/users` и `/users/` в OpenAPI;
- P101: два параллельных контура самостоятельной смены email объединены в один: страница безопасности аккаунта и профиль используют `/dashboard/profile/email-change/request` и `/dashboard/profile/email-change/cancel`;
- старые `/account/email/change-request` удалены из backend, поэтому блокировка пользователя, ограничение повторной отправки, отзыв токенов подтверждения и журналирование больше не расходятся между двумя реализациями;
- backend-тест закрепляет отсутствие старого маршрута и наличие канонического, frontend-проверка контракта запрещает возвращать старый адрес API; затронутые комментарии и названия групп OpenAPI приведены к русскому системному контракту;
- из auth handler удалены ручные `db_session.rollback()`; regression-тесты закрепляют savepoint rollback для duplicate, ошибки базовой роли и отсутствующего demo-тарифа;
- P90: availability preflight регистрации строго валидирует JSON/email/телефон/ИНН и возвращает `400` вместо необработанного `500` на malformed input;
- телефон нормализуется в тот же формат `+7XXXXXXXXXX`, который хранится после регистрации, поэтому занятый номер больше не может ошибочно считаться свободным;
- frontend availability-check для email/телефона/ИНН работает fail-closed при сетевой ошибке, а номер телефона больше не выводится в browser console;
- frontend contract test закрепляет fail-closed UX и отсутствие PII debug-log;
- P91: email пользователя становится единым case-insensitive идентификатором — новые регистрации сохраняют lowercase, а поиск/login/reset находят аккаунт независимо от регистра ввода;
- SQLAlchemy metadata и PostgreSQL получают уникальный индекс по `lower(email)`, исключающий создание двух аккаунтов, различающихся только регистром;
- миграция fail-closed проверяет существующие case-дубликаты и требует разрешить их вручную вместо автоматического слияния или удаления аккаунтов;
- исторический регистр существующих email не переписывается: изменение ограничено уникальностью и канонизацией новых записей;
- P92: из регистрации юридического лица удалены поля расчётного счёта и БИК, которые backend-модель не сохраняла и система нигде не использовала;
- registration payload больше не содержит несохраняемые банковские реквизиты, поэтому пользователь не вводит данные, которые затем молча теряются;
- frontend contract test блокирует возврат этих полей без реального backend-контракта хранения;
- P93: `/auth/registration` строго валидирует JSON-структуру и поля на backend, а malformed payload больше не доходит до транзакции;
...
- P94: ИНН пользователя получает DB-уникальность на уровне непустого `trim(inn)`, чтобы параллельные регистрации и admin-edit не могли обойти preflight-проверку;
- lookup и insert нормализуют внешние пробелы ИНН, а миграция fail-closed останавливается при исторических дублях после `trim`;
- partial functional index не ограничивает пользователей без ИНН и не переписывает исторические значения автоматически;
...
- P97: завершён self-service flow подтверждаемой смены email через существующие `pending_email`, durable outbox и одноразовые verification-токены;
- новый адрес остаётся `pending_email` до подтверждения, текущий email продолжает использоваться для входа, а после подтверждения существующий verification-flow ротирует `session_version` и отзывает неиспользованные reset-токены;
- запрос смены email валидирует JSON и адрес, блокирует уже занятый email, fail-closed проверяет готовность verification-mail и не помещает адрес в открытом виде в idempotency key/lifecycle evidence;
- смена pending-адреса сериализуется блокировкой строки пользователя, сразу отзывает старые неиспользованные verification-токены и использует реальный resend-throttle по durable outbox;
- профиль показывает ожидающий подтверждения адрес, позволяет повторно отправить письмо и отдельно отменить pending-смену; отмена также отзывает verification-токены;
- телефон честно помечен как изменяемый только администратором до появления отдельного подтверждаемого phone-flow;
- frontend contract test блокирует возврат ложной подсказки о self-service подтверждении телефона и потерю cancel-flow;
- P98: подтверждение email переведено на nested savepoint — конкурентный конфликт адреса остаётся стабильным `409 EMAIL_ALREADY_EXISTS`, но handler больше не делает ручной rollback request-сессии;
- confirm/resend строго принимают JSON-объект с разрешёнными полями, malformed/unsupported payload и невалидные token/email получают предсказуемый `400` до транзакционного действия;
- resend использует общий email-normalizer и безопасный интервал `max(1, ...)`, поэтому ошибочная нулевая настройка не вызывает деление на ноль;
- OpenAPI-tag email verification и затронутые комментарии приведены к русскому системному контракту, regression-тест блокирует возврат ручного rollback.
