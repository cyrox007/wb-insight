...
const ROLE_DESCRIPTIONS = {
  ...
  manager: 'Операционная роль: просмотр пользователей, тарифов, платежей, рассылок и аудита без права изменять критические данные.',
  support: 'Поддержка клиентов: чтение пользовательских карточек и аудита без административных write-операций.',
  analyst: 'Безопасная операционная сводка без доступа к персональным карточкам пользователей и критическим настройкам.',
    ...
    <template v-else>
      <section class="rbac-role-summary">
        <article v-for="role in roles" :key="role.code" class="cp-card rbac-role-card">
          ...
          <div v-else class="cp-muted">Дополнительных административных permissions нет.</div>
        </article>
      </section>
