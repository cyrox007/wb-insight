...
- P84 безопасное permanent delete пользователей подготовлено поверх текущего `dev`: отдельное разрешение `users:delete` доступно только `admin/super_admin`, self-delete и удаление активного аккаунта запрещены.
