...
- ошибки DB flush при удалении WB-подключения не скрываются: транзакционная граница запроса выполняет rollback, а общий обработчик `500` возвращает безопасное русское сообщение без деталей исключения;
