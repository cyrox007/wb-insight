...
- P92 строгий server-side контракт регистрации подготовлен поверх текущего `dev`: malformed payload и некорректные поля получают 400 до savepoint/БД.
...
- P92 post-candidate registration input hardening валидирует структуру JSON и поля регистрации на backend, канонизирует email/телефон и не допускает malformed/oversized значения до транзакции и bcrypt;
