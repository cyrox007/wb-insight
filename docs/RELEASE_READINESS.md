...
- клиентское состояние кабинетов очищается при выходе и защищено от запоздалых ответов предыдущей сессии, поэтому данные одного пользователя не переиспользуются после входа другого пользователя без перезагрузки страницы;
