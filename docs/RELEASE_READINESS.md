...
- P99 registration integrity hardening преобразует в 409 только известные identity unique-constraints email/phone/ИНН; остальные integrity failures не маскируются и остаются внутренними ошибками общего request-контура.
...
- P99 post-candidate registration integrity hardening отличает реальные duplicate identity conflicts от посторонних ошибок целостности: только известные уникальные индексы email/телефона/ИНН дают стабильный 409, остальные IntegrityError проходят в общий безопасный 500-handler;
