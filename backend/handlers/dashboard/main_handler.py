...
def _calc_change(current: float, previous: float) -> tuple[float, float]:
    ...
    change_percent = change_abs / previous * 100 if previous else (100.0 if current > 0 else 0.0)
...
async def _calculate_stats(
    ...
    base_result = await get_base_report_stats(session, user_id, start_date, end_date, scope)
    sales_result = await get_sales_report_stats(session, user_id, start_date, end_date, scope)
    returns_result = await get_returns_report_stats(session, user_id, start_date, end_date, scope)
    unit_economy = await get_dashboard_unit_economy_scoped(session, user_id, start_date, end_date, scope)
    orders = await get_order_totals(session, user_id, start_date, end_date, scope)
    advertising = await get_advertising_totals(session, user_id, start_date, end_date, scope)
    ...
    forecast = fact_current_month / days_in_period * days_in_month if days_in_period else fact_current_month
    recommended_orders_per_day = (
        (plan_current_month - fact_current_month) / max(1, days_in_month - days_in_period)
        if days_in_month > days_in_period
        else 0.0
    )

    prev_start_date, prev_end_date = previous_period(start_date, end_date)
    prev_base_result = await get_base_report_stats(session, user_id, prev_start_date, prev_end_date, scope)
    prev_sales_result = await get_sales_report_stats(session, user_id, prev_start_date, prev_end_date, scope)
    prev_returns_result = await get_returns_report_stats(session, user_id, prev_start_date, prev_end_date, scope)
    prev_unit_economy = await get_dashboard_unit_economy_scoped(session, user_id, prev_start_date, prev_end_date, scope)
    prev_orders = await get_order_totals(session, user_id, prev_start_date, prev_end_date, scope)
    ...
    cart_rate = advertising.added_to_cart / advertising.clicks * 100 if advertising.clicks else 0.0
...
async def _resolve_scope_or_error(session: AsyncSession, user_id: UUID, token_id: UUID | None):
    try:
...
async def dashboard(
    ...
    if not scope.token_ids:
        if auth_errors:
            return response_error(
                message="Ошибка авторизации: подключение недействительно или истекло. Обновите кабинет Wildberries в настройках.",
                code="TOKEN_INVALID",
            )
        return response_error(
    ...
    if not has_any_success and sync_errors:
        return response_error(message=f"Ошибка синхронизации: {sync_errors[0]}", code="SYNC_ERROR")
    if not has_any_success:
        return response_error(message="Данные отсутствуют → синхронизация не запускалась", code="NOT_SYNCED")
    if await has_active_sync_jobs(db_session, user_id, scope):
        return response_success(is_synced=False, is_syncing=True, message="Идёт синхронизация данных", stats={}, partial=True)

    stats_data = await _calculate_stats(db_session, user_id, start_date, end_date, scope)
...
async def dashboard_charts(
    ...
    chart_data = await get_chart_data(db_session, user_id, start_date, end_date, scope)
    warehouse_data = await get_warehouse_data(db_session, user_id, scope)
    abc_analysis = await get_abc_analysis(db_session, user_id, start_date, end_date, scope)
    category_data = await get_category_data(db_session, user_id, start_date, end_date, scope)
    size_chart = await get_size_chart(db_session, user_id, start_date, end_date, scope)
