...
from services.wb_report_service import check_wb_report_stats, get_base_wb_report_stats, get_returns_wb_report_stats, get_sales_wb_report_stats, get_chart_data, get_warehouse_data, get_category_data, get_abc_analysis, get_size_chart
...
async def dashboard(
    ...
    # Расчёт метрик для fact_current_month, plan_current_month и т.д.
    days_in_period = (end_date - start_date).days if end_date and start_date else 30
    days_in_month = 30  # упрощённо

    # fact_current_month = выручка за текущий месяц
    fact_current_month = revenue

    # plan_current_month = план на месяц (можно брать из настроек или рассчитывать)
    # пока используем план как факт * 1.2 (цель роста 20%)
    plan_current_month = revenue * 1.2 if revenue > 0 else 50000000

    # done = процент выполнения плана
    ...
    # recommended_orders_per_day = рекомендуемое количество заказов в день
    ...
    # Расчёт метрик за предыдущий период
    ...
    prev_sales_amount = float(getattr(prev_sales_result, 'sales_amount', 0) or 0)
    prev_sales_units = int(getattr(prev_sales_result, 'sales_units', 0) or 0)

    prev_returns_amount = float(getattr(prev_returns_result, 'returns_amount', 0) or 0)
    prev_returns_units = int(getattr(prev_returns_result, 'returns_units', 0) or 0)

    prev_revenue = prev_sales_amount - prev_returns_amount
    prev_to_pay = float(getattr(prev_base_result, 'to_pay', 0) or 0)

    # Unit-экономика за предыдущий период
    ...
    def calc_change(current: float, previous: float) -> tuple[float, float]:
    ...
    # Статистика
    ordered_amount_change_abs, ordered_amount_change_percent = calc_change(ordered_amount, prev_ordered_amount)
    ordered_units_change_abs, ordered_units_change_percent = calc_change(ordered_units, prev_ordered_units)
    revenue_change_abs, revenue_change_percent = calc_change(revenue, prev_revenue)
    sales_units_change_abs, sales_units_change_percent = calc_change(sales_units, prev_sales_units)
    to_pay_change_abs, to_pay_change_percent = calc_change(to_pay, prev_to_pay)
    profit_change_abs, profit_change_percent = calc_change(profit, prev_profit)
    buyout_rate_change_abs, buyout_rate_change_percent = calc_change(buyout_rate, prev_buyout_rate)
    avg_price_change_abs, avg_price_change_percent = calc_change(avg_price, prev_avg_price)
    stats = {
        "ordered_amount": {
        ...
        "fact_current_month": {
            "value": round(fact_current_month, 2)
        },
        "plan_current_month": {
            "value": round(plan_current_month, 2)
        },
        "done": {
            "value": round(done, 2)
        },
        "forecast": {
            "value": round(forecast, 0)
        },
    ...
    # данные для графика - получаем из БД
    chartData = await get_chart_data(
        session=db_session,
        user_id=current_user['sub'],
        start_date=start_date,
        end_date=end_date
    ...
    # основные показатели по кабинету - получаем из БД
    base_stats = {
    ...
    # данные по складам - получаем из БД
    warehouseData = await get_warehouse_data(
        session=db_session,
        user_id=current_user['sub']
    )

    # ABC анализ - получаем из БД
    abcAnalysis = await get_abc_analysis(
        session=db_session,
        user_id=current_user['sub'],
        start_date=start_date,
        end_date=end_date
    ...
    # данные по категориям - получаем из БД
    categoryData = await get_category_data(
        session=db_session,
        user_id=current_user['sub'],
        start_date=start_date,
        end_date=end_date
    ...
    # данные по размерам - получаем из БД
    sizeChart = await get_size_chart(
        session=db_session,
        user_id=current_user['sub'],
        start_date=start_date,
        end_date=end_date
